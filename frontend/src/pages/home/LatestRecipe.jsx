...
const LatestRecipe = () => {
    ...
    useEffect(() => {
        const getLatestItems = async () => {
            const response = await axios.get(`https://food-website-api.vercel.app/api/all-items`);
  ...
  return (
    <div className='px-5 xl:px-10 py-16'>
            <h2 className='text-3xl font-semibold text-secondary sm:text-5xl sm:leading-relaxed'>
                ...
                {/* get all items */}
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-col-3 xl:grid-cols-4 gap-8'>
                    {
                ...
                <button className='py-4 px-8 hover:bg-btnColor text-secondary hover:text-white
                    ...
                    border-[#9c702a] focus:outline-none rounded-lg'>View All Recipe</button></Link>
                </div>
